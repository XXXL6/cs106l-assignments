...
void parse_csv(std::string filename, std::vector<Course> &courses) {
  ...
  if (file.is_open()) {
    ...
    while (std::getline(file, temp)) {
      ...
      std::vector<std::string> a = split(temp, ',');
      Course b{a[0], a[1], a[2]};
      courses.push_back(b);
    }
  }
  file.close();
...
void write_courses_offered(std::vector<Course> &all_courses) {
  /* (STUDENT TODO) Your code goes here... */
  std::vector<Course> temp1;
  std::vector<Course> temp2;
  for (auto &all_course : all_courses) {
    if (all_course.quarter != "null")
      temp1.push_back(all_course);
    else
      temp2.push_back(all_course);
  }
  all_courses = std::move(temp2);
  std::ofstream file;
  file.open(COURSES_OFFERED_PATH, std::ios::out);
  file << "Title,Number of Units,Quarter" << '\n';
  for (auto &all_course : temp1) {
    file << all_course.title << ',' << all_course.number_of_units << ','
         << all_course.quarter << '\n';
  }
  file.close();
}
...
void write_courses_not_offered(const std::vector<Course> &unlisted_courses) {
  ...
  file << "Title,Number of Units,Quarter" << '\n';
  for (auto &all_course : unlisted_courses) {
    file << all_course.title << ',' << all_course.number_of_units << ','
         << all_course.quarter << '\n';
  }
  file.close();
